...
export default function PostUpload() {
  ...
  const handlePost = async (event) => {
    ...
    if (file) {
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL = null) => {
            try {
              createPost(userData.userId, caption, downloadURL).then(() => {
      ...
      try {
        createPost(userData.userId, caption, attchment).then(() => {
          history.push(ROUTES.DASHBOARD);
        });
      } catch (e) {
        console.log(e.message);
  ...
  return (
    <>
      ...
      <form
        className="flex w-full p-2 justify-around"
        onSubmit={handlePost}
        method="POST"
      >
        <img
        ...
        <div className="flex flex-col w-full">
          <textarea
            className="flex-1 px-2 dark:text-white dark:bg-gray-600 rounded-lg"
            type="text"
            rows="4"
        ...
        <div className="px-2">
          <button
            className="px-1 dark:text-white dark:bg-gray-600 rounded-sm"
            type="submit"
